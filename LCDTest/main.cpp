...
#define DEMO_FOR_WIFI_LED 0
#define WIFI_SCAN_SSID_PRNT 0
#define DEMO_FOR_LCD_C12832 1

#if DEMO_FOR_LCD_C12832
C12832 lcd(SPI_MOSI/* PE_14 */, SPI_SCK/* PE_12 */, PD_12, PD_11, SPI1_CS/* PE_9 */); //LCD: MOSI, SCK, RESET, A0, nCS
Thread threadlcd;
void lcd_display() {
    int j = 0;
    lcd.cls();
    lcd.locate(0, 3);
    lcd.printf("pchen arm mbed board!");
...
Thread threadled1, threadled2, threadled3, threadled4, threadledrgb;
...
bool ssid_find(const char *ssid)
{
    ...
    printf("Total %d networks available. We need to found %s SSID\n", count, ssid);
...
#if DEMO_FOR_RTOS_THREAD
    threadled1.start(led1_blink);
    threadled2.start(led2_blink);
    threadled3.start(led3_blink);
    //threadled4.start(led4_blink);
    threadled4.start(callback(blink, &led4));
    threadledrgb.start(ledred_blink);
#endif

#if DEMO_FOR_WIFI_LED
    threadwifi.start(wifi_connect);
#endif


#if DEMO_FOR_LCD_C12832
    threadlcd.start(lcd_display);
#endif
